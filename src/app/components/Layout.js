...
export default function Layout({
  ...
  // Helper to get attachments for a specific note
      ...
      <div className="flex-1 flex flex-col">
        <header className="flex items-center justify-between bg-white shadow p-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <FiLogOut
              className="text-gray-700 w-6 h-6 cursor-pointer hover:text-red-500"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {notes.map((note) => (
                  <NoteCard
                    ...
                    role="owner"
                    attachments={getNoteAttachments(note.id)}
                ...
                sharedByMeNotes
                  ...
                  .map((item) => (
                    <NoteCard
                      ...
                      role="owner"
                      attachments={getNoteAttachments(item.note_id.id)}
                      onEdit={() => openCreateModal(item.note_id)}
                      onDelete={() => onDeleteNote(item.note_id)}
                    />
                  ))
              ) : (
                <p className="text-gray-500">You haven't shared any notes.</p>
              )}
            </div>
          )}
              ...
              {activityLogs.length > 0 ? (
                activityLogs.map((log, i) => (
                  <div key={i} className="p-2 border rounded bg-white">
                    {log.isOwner ? (
                      <p>
                        You shared <strong>{log.noteTitle}</strong> with{" "}
                        <strong>{log.recipientEmail}</strong> on{" "}
                        {new Date(log.sharedAt).toLocaleString()}
                      </p>
                    ) : (
                      <p>
                        You received note <strong>{log.noteTitle}</strong> from{" "}
                        <strong>{log.ownerEmail}</strong> on{" "}
                        {new Date(log.sharedAt).toLocaleString()}
                      </p>
                    )}
                  </div>
